...
def main() -> None:
    with isolated_server() as server:
        ...
        expect(run.get("status") == "success", "Le run iterator actif doit réussir.")
        expect(run.get("iterator_cursors", {}).get("iterator-1") == 2, "Le curseur iterator attendu est 2.")
        item = run.get("output_values", {}).get("iterator-1:1", {})
        expect(json.loads(str(item.get("value") or "{}")) == {"item": "beta"}, "Le dernier item doit être beta.")
        expect(run.get("output_values", {}).get("iterator-1:2", {}).get("value") == "1", "Le dernier index doit être 1.")
