...
def main() -> None:
    with isolated_server() as server:
        ...
        expect(run.get("status") == "success", "The iterator run with visual feedback must succeed.")
        expect(run.get("iterator_cursors", {}).get("iterator-1") == 3, "The expected iterator cursor is 3 after a bounded feedback loop.")
